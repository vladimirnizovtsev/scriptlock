...
Please report vulnerabilities in Tessera privately through GitHub security advisories:

https://github.com/vladimirnizovtsev/tessera/security/advisories/new

Do not open a public issue or pull request for a vulnerability. Include the version (`npx tessera --version`), a minimal reproduction and the impact you see. You will get an acknowledgement within seven days and a fix or a written assessment as soon as reasonably possible; the advisory is published together with the fixed release. There is no bug bounty. Credit is given in the advisory and the changelog unless you ask otherwise.

## Scope

In scope:

- The `tessera-cli` npm package: the CLI, the collector, identity and integrity logic, manifest handling and reporting.
- The composite GitHub Action in `action.yml` and the example workflows.
- Ways to make Tessera report a wrong inventory or a clean diff for a page that changed, for example a script that evades collection, an identity collision, a manifest match that is broader than intended, or a `sourceURL` trick that is not flagged as spoofed. These are the bugs that matter most for this project.
...
- Vulnerabilities in websites you scan with Tessera. Report those to the site owner.
- Vulnerabilities in Chromium, Playwright or other dependencies. Report those upstream; a dependency bump in Tessera is welcome as a pull request.
...
- Bot management blocking a scan. Tessera deliberately ships no stealth patches.
...
Tessera runs entirely on the machine that executes it. It does not send snapshots, manifests, headers or script sources anywhere. Snapshots never contain script source text, but they do contain URLs, hashes, headers and the vantage point (user agent, browser build, and the host name of the scanning machine); treat `.tessera/` and the run artifacts with the same care as the rest of your build output.
